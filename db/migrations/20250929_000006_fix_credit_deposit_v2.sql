...
-- Force update by running this new migration
-- Also ensures players table is in realtime publication
...
begin
  ...
  if v_player_id is null then
    -- Try to look up by wallet using the correct column 'wallet_address'
...
-- Add players table to supabase_realtime publication if not present
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
